...
fasta=${1}
cm_db="/hps/nobackup2/production/metagenomics/aalmeida/databases/rfam_cms/archaea/"
...
parse_rRNA-archaea.py ${fasta%%.f*}"_"all.tblout.deoverlapped > ${fasta%%.f*}"_"rRNAs.out
rRNA2seq.py ${fasta%%.f*}"_"all.tblout.deoverlapped ${fasta} > ${fasta%%.f*}"_"rRNAs.fasta

echo "[ Detecting tRNAs ]"
tRNAscan-SE -A -Q -m ${fasta%%.f*}_stats.out -o ${fasta%%.f*}_trna.out ${fasta}
